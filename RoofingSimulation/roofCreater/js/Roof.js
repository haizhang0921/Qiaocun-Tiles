...
//			console.log(this.tongPool.tiles);
		...
		this.runFirstRowA=function(map,pool,usedCallback,banMap){
			...
			for(var col=0;col<colCount;col++){
				...
				if(pool.type==Tile.TONGWA){
					tileInfo = pool.getCanUsed(null,this,banMap,col,row);
				}else{
					tileInfo=pool.shift();
				}
				if(tileInfo){
		...
		this.runOneColA=function(map,col,pool,usedCallback,banMap){
			...
			for(var row=1;row<rows;row++){
				...
				var tileInfo = pool.getCanUsed(prev.tile,this,banMap,col,row);
				if(tileInfo){
					tileInfo.row=row;
					tileInfo.col=col;
					tileInfo.type="A";
		...
		this.run = function(usedCallback){
			...
			this.runFirstRowA(this.banMapA,this.banPool,usedCallback);
			this.banMapB=new Array(cols);
			this.runFirstRowB(this.banMapB,this.banPool,usedCallback);
			
			for(var col=0;col<cols;col++){
				this.runOneColA(this.banMapA,col,this.banPool,usedCallback);
				this.runOneColB(this.banMapB,cols-col-1,this.banPool,usedCallback);
			}
