function TilePool(options){
	...
	this.create= function(count){
		...
		if(this.type==Tile.BANWA){
			...
			console.log("筒瓦池:共"+count+"块，分为："+this.groupCount+"组【"+groupList.join("、")+"】，各组占比情况------------");
		}
		for(var groupName in TileCreator.GROUP_LEVELS){
			var list = this.groupMap[groupName];
	...
	this.randomTileByGroupNames=function(groupNames){
		...
		if(count>0){
			...
			console.error("分组内已没有瓦："+groupNames.join("、")+",使用随机取瓦！");
...
//	}
	...
	this.backPushTile=function(tile){
		tile.list.push(tile);
		this.tiles.push(tile);
	}
...
//			console.log(smallName+":"+count);
	...
	this.getCanUsed = function(oldTileInfo,tile,roof,banMap,col,row,leftCol){
		var failInfos=[];
		...
		for(var i=0;i<count;i++){
			...
			if(canUse.status===true){
				...
				if(oldTileInfo){
					oldTileInfo.tryCount=i;
					oldTileInfo.tile=nextTile;
					oldTileInfo.replaceCount+=1;
					oldTileInfo.failInfos=oldTileInfo.failInfos.concat(failInfos);
					if(oldTileInfo.replaceCount>1){
			...
			if(i==this.maxTryCount){
				var ret=new TileInfo({tryCount:i,failInfos:failInfos});
				return ret;
			}
		}
		...
		for(var i=0;i<allCount;i++){
			...
			if((count+i)==this.maxTryCount){
				var ret=new TileInfo({tryCount:count+i,failInfos:failInfos});
				return ret;
			}
		}
		if(oldTileInfo){
			oldTileInfo.tryCount=count+allCount;
			delete oldTileInfo.tile;
			oldTileInfo.replaceCount+=1;
			oldTileInfo.failInfos=oldTileInfo.failInfos.concat(failInfos);
			if(oldTileInfo.replaceCount>1){
				oldTileInfo.isStop = true;
...
function TileInfo(options){
	...
	this.getFailCount=function(type){
		var count=0;
		for(var i=0;i<this.failInfos.length;i++){
			if(this.failInfos[i].type==type){
				count++;
			}
		}
		return count;
	...
	this.toSimpleString=function(){
		...
		if(this.replaceCount>0){
			html+='<span class="red">'+this.replaceCount+'</span><br>';
		}else{
			html+='<span >'+this.replaceCount+'</span><br>';
		}
		if(!tile){
